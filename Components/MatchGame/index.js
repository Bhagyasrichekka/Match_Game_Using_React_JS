...
class MatchGame extends Component {
  ...
  increaseTimer = () => {
    ...
    if (timer === 0) {
      this.clearTimer()
      this.setState({isGameActive: false, timer: 0})
    } else {
  ...
  onClickImg = imgId => {
    const {gameImg} = this.state
    const {id} = gameImg

    if (imgId === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        gameImg: imagesList[Math.ceil(Math.random() * imagesList.length)],
      }))
    } else {
      this.setState({isGameActive: false, timer: 0})
      this.clearTimer()
  ...
  renderTheGame = () => {
    const {gameImg, activeTabId} = this.state
    const {imageUrl} = gameImg
    const activeTabImgsList = imagesList.filter(
      eachItem => eachItem.category === activeTabId,
    )

    return (
      <div className="bg">
        <img src={imageUrl} alt="match" className="game-img" />
  ...
  render() {
    const {timer, score, isGameActive} = this.state
    return (
      <div className="bg">
        <NavBarItem timer={timer} score={score} key="Nav" />
        {isGameActive ? this.renderTheGame() : this.renderScoreCard()}
      </div>
    )
  }
}
